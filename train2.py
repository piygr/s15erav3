...
def train(config):
    ...
    '''
    train_dataloader =  DataLoaderLite(B=config['tokens']['batch_size'],
                                       T=config['model']['model_config']['sequence_length'],
                                       tokenizer=tokenizer)
    ...
    sample_prompt = "Kohli comes forward and "

    for step in range(config['tokens']['train_steps']):
        t0 = time.time()
        if step >= max_steps:
        ...
        batch = train_dataloader.next_batch().to(device)
